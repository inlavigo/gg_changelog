...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_changelog.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggChangelog = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_changelog.dart',
      );

      expect(
        ggChangelog,
        isNotNull,
        reason: 'Item for executing bin/gg_changelog.dart not found',
      );

      expect(ggChangelog['name'], 'gg_changelog.dart');
      expect(ggChangelog['type'], 'dart');
      expect(ggChangelog['request'], 'launch');
